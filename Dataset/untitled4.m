...
initialEnergy = 2.0; % Initial energy for each node in Joules
...
residualEnergy = ones(numNodes, 1) * initialEnergy; % Initial energy for each node
...
disp('Best Cluster Head Solution:');
disp(bestSolution);
...
scatter(bestCHPosition(1), bestCHPosition(2), 100, 'r', 'filled', 'DisplayName', 'Best Cluster Head');
...
function plot_results(metrics, rounds_range, nodes_range)
    % Full Implementation of plotting as per paper
    % Full Implementation of plotting as per paper
    figure;

    % Plot 1: Network Lifetime
    figure;
    plot(metrics.alive, '-o', 'LineWidth', 1.5);
    ...
    figure;
    plot(metrics.delay, '-s', 'LineWidth', 1.5);
    ...
    figure;
    plot(metrics.energy, '-d', 'LineWidth', 1.5);
    ...
    % Plot 4: Throughput
    figure;
    plot(metrics.throughput, '-s', 'LineWidth', 1.5);
    title('Throughput');
    xlabel('Rounds');
    ylabel('Packets/s');
    grid on;

    % Plot 5: Packet Delivery Ratio (PDR)
    figure;
    plot(metrics.pdr, '-o', 'LineWidth', 1.5);
